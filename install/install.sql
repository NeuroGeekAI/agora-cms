...
CREATE TABLE IF NOT EXISTS ag_articles (
    ...
    une         TINYINT(1) DEFAULT 0,
    vues        INT UNSIGNED DEFAULT 0,
    partages    INT UNSIGNED DEFAULT 0,
    access_score TINYINT UNSIGNED DEFAULT 100,
...
CREATE TABLE IF NOT EXISTS ag_debats (
    id          INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    titre       VARCHAR(255) NOT NULL,
    description TEXT,
    pour        TEXT,
    contre      TEXT,
    votes_pour  INT UNSIGNED DEFAULT 0,
    votes_contre INT UNSIGNED DEFAULT 0,
    statut      ENUM('ouvert','ferme','archive') DEFAULT 'ouvert',
    featured    TINYINT(1) DEFAULT 0,
    created_at  DATETIME DEFAULT CURRENT_TIMESTAMP
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
CREATE TABLE IF NOT EXISTS ag_debat_votes (
    id         INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    debat_id   INT UNSIGNED NOT NULL,
    position   ENUM('pour','contre') NOT NULL,
    ip_hash    VARCHAR(64) NOT NULL,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (debat_id) REFERENCES ag_debats(id) ON DELETE CASCADE,
    UNIQUE KEY unique_vote (debat_id, ip_hash)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
('Equipe Editoriale', 'La redaction officielle du site.', 'admin');
INSERT IGNORE INTO ag_settings (cle, valeur) VALUES
('site_name',        'AgoraCMS'),
('site_tagline',     'La plateforme de votre mouvement'),
('maintenance',      '0'),
('ga_id',            ''),
...
('lois',           1, 'Lois francaises expliquees'),
('manifeste',      1, 'Programme & Manifeste'),
('debats',         1, 'Debats en ligne'),
('petitions',      1, 'Petitions citoyennes'),
('rejoindre',      1, 'Rejoindre le mouvement'),
('proteger_docs',  1, 'Protection de documents'),
('souverainevoix', 1, 'Audio Accessibilite');
INSERT IGNORE INTO ag_articles (titre, slug, extrait, contenu, categorie, auteur_id, statut, une, image_alt, access_score) VALUES
('Bienvenue sur votre plateforme', 'bienvenue-agoracms', 'Votre plateforme est prete. Personnalisez-la depuis l administration.', '<h2>Votre plateforme est prete !</h2><p>Felicitations pour l installation d AgoraCMS. Publiez vos articles, creez des petitions, organisez des debats.</p>', 'programme', 1, 'publie', 1, 'Image de bienvenue', 95);
